...
void auto_wp_quals() {
  ...
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set({{-46, 28, -90}, fwd, DS});
  intakeLS.move(127);
  chassis.pid_wait_quick_chain();
...
void left_side_elims() {
  ...
  chassis.pid_wait_quick_chain();
  pros::delay(1500);
  chassis.pid_odom_set({{-18.5, 19, 0}, fwd, DS});
  trapdoor.set(false);
  chassis.pid_wait_quick_chain();
...
void right_side_elims() {
  ...
  chassis.pid_odom_set(-3, DS);
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set({{19.5, 15, 180}, fwd, DS});
  chassis.pid_wait_quick_chain();
...
void skills() {
  ...
  chassis.pid_odom_set(5, DS);
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set({{-24, 63, 135}, fwd, 70});
  intakeUS.move(-127);
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set(9, DS/3);
  chassis.pid_wait_quick_chain();
  pros::delay(300);
  intakeLS.move(-80);
  pros::delay(2000);
  ...
  chassis.pid_wait_quick_chain();
  trapdoor.set(false);
  chassis.pid_odom_set({{43, 96, 0}, fwd, DS});
  chassis.pid_wait_quick_chain();
  pros::delay(1000);
  chassis.pid_odom_set(-10, 127);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set({{1, 10, 180}, fwd, DS});
  intakeLS.move(127);
  intakeUS.move(127);
  chassis.pid_wait_quick_chain();
  aligner.set(true);
