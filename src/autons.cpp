 ...
 */
 ...
 chassis.pid_turn_set(135_deg, TS);
 chassis.pid_wait();
 chassis.pid_drive_set(-20_in, DS);
 chassis.pid_wait();
 chassis.pid_drive_set(-6_in, DS/2);
 chassis.pid_wait();
 goalClamp.set(true);
 intake.move(127);
 chassis.pid_drive_set(34_in, DS);
 chassis.pid_wait();
 chassis.pid_drive_set(20_in, DS/1.5);
 chassis.pid_wait();
 pros::delay(100);
 chassis.pid_drive_set(-8_in, DS);
 chassis.pid_wait();
 goalrush.set(true);
 chassis.pid_turn_set(45_deg, TS/2);
 chassis.pid_wait();
 goalrush.set(false);
 chassis.pid_turn_set(135_deg, TS);
 chassis.pid_wait();
 ...
 chassis.pid_wait();
 goalrush.set(false);
 chassis.pid_swing_set(ez::RIGHT_SWING, -45_deg, SS, ez::ccw);
 chassis.pid_wait();
 chassis.pid_drive_set(-14_in, DS);
 goalClamp.set(false);
}
...
void skills() {
  ...
  chassis.pid_swing_set(ez::LEFT_SWING, 98_deg, SS);
  chassis.pid_wait();
  //Drive forward
